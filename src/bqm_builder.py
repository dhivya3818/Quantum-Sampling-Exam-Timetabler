...
class BqmBuilder:
    ...
    def get_one_hot_constraints(self):
        ...
        return [dimod.generators.combinations(
            [f'v_{exam},{k}' for k in range(self.num_days)], 1) for exam in self.graph.nodes]
        ...
        for (i, j) in self.graph.edges:
            for day in range(self.num_days):
                for ind, weight in enumerate(self.weights):
                    ...
                    J[f'v_{i},{day}', f'v_{j},{new_day}'] += (
                        weight * self.graph[i][j]["weight"]) / self.num_students
                    J[f'v_{j},{day}', f'v_{i},{new_day}'] += (
                        weight * self.graph[i][j]["weight"]) / self.num_students
    ...
    def get_classroom_constraints(self):
        ...
        def check_classroom_constraints(comb):
            ...
            return all([self.graph.nodes[exams[i]]['size'] <=
                       self.resources['classrooms'][i] for i in range(len(exams))])
        ...
        for day in range(self.num_days):
            ...
            combs = list(combinations([f'v_{exam},{day}' for exam in range(
                self.num_exams)], len(self.resources['classrooms'])))
            for comb in combs:

                if not check_classroom_constraints(list(comb)):
                    valid_configurations = set(
                        product([0, 1], repeat=3)) - {(1, 1, 1)}
